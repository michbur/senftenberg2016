...
<<qpcr2pp, echo = FALSE, message=FALSE,fig.width=13, fig.height=8>>=
...
ggplot(plot_data, aes(y = exprep, x = lambda, colour = experiment, ymin = exprep, ymax = exprep)) + 
  ...
  scale_y_discrete("Replicate\n", label = plot_data[["assay"]]) + 
  scale_x_continuous(expression(lambda)) + 
  scale_color_discrete("Experiment name:") + 
  geom_errorbarh(aes(x = lambda, xmin = lambda.low, xmax = lambda.up), size = 1.2, height = nlevels(plot_data[["exprep"]])/160) +
  facet_wrap(~ method) +
  my_theme
@
...
plot_data[["exprep"]] <- plot_data[["assay"]]
...
\citep{dorazio_statistical_2015}.
